class FavoritesController < ApplicationController
  def create
    ...
    # photoの詳細画面に遷移する
    redirect_to photo_path(@favorite_photo.id), notice: "#{favorite.photo.user.name}さんのブログをお気に入り登録しました"
  ...
  def destroy
    ...
    # photoの詳細画面に遷移する
    redirect_to photo_path(@photo.id), notice: "#{favorite.photo.user.name}さんのブログをお気に入り解除しました"
  end
